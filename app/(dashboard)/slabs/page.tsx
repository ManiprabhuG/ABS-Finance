...
export default function SlabsPage() {
  ...
  // Form State — 4 Required Fields each (ltvRange, minLtv, maxLtv, interestRate) / (name, fromAmount, toAmount, interestRate)
  const [ltvForm, setLtvForm] = useState({ ltvRange: '', minLtv: '', maxLtv: '', interestRate: '', status: 'ACTIVE' });
  const [intForm, setIntForm] = useState({ name: '', fromAmount: '', toAmount: '', interestRate: '', status: 'ACTIVE' });
  ...
  const handleAddLtvSlab = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setLtvForm({ ltvRange: '', minLtv: '', maxLtv: '', interestRate: '', status: 'ACTIVE' });
        setEditingLtv(null);
        fetchSlabs();
      } else {
        const err = await res.json();
        alert(err.error || 'Failed to save LTV slab');
      }
    } catch (e: any) {
      alert(e.message);
    }
  ...
  const handleAddIntSlab = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editingInt ? `/api/slabs/interest/${editingInt.id}` : '/api/slabs/interest';
      const method = editingInt ? 'PUT' : 'POST';
      ...
      if (res.ok) {
        setIntForm({ name: '', fromAmount: '', toAmount: '', interestRate: '', status: 'ACTIVE' });
        setEditingInt(null);
        fetchSlabs();
      } else {
        const err = await res.json();
        alert(err.error || 'Failed to save Amount Interest slab');
      }
    } catch (e: any) {
      alert(e.message);
    }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-800 text-slate-500 text-xs uppercase font-semibold">
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end space-x-1">
                        <button
                          ...
                          className="p-1.5 rounded-lg text-brand-600 hover:bg-slate-100 dark:hover:bg-slate-800"
                          title="View Details"
                        >
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-lg text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-950/60"
                          title="Edit Slab"
                        >
          ...
          {/* Add/Edit LTV Form — EXACT 4 REQUIRED FIELDS */}
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-4">
            ...
            <form onSubmit={handleAddLtvSlab} className="space-y-3 text-xs">
              <div>
                <label className="block font-semibold text-slate-600 mb-1">LTV Range Name * (Req #1)</label>
                <input
                  type="text"
                  required
                  placeholder="e.g. Low LTV (0-40%)"
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block font-semibold text-slate-600 mb-1">Min LTV % * (Req #2)</label>
                  <input
                    type="number"
                    required
                    placeholder="Min LTV %"
                ...
                <div>
                  <label className="block font-semibold text-slate-600 mb-1">Max LTV % * (Req #3)</label>
                  <input
                    type="number"
                    required
                    placeholder="Max LTV %"
              ...
              <div>
                <label className="block font-semibold text-slate-600 mb-1">Interest Rate % p.a. * (Req #4)</label>
                <input
                  type="number"
                  step="0.1"
                  required
              ...
              <div className="flex space-x-2 pt-2">
                {editingLtv && (
                  <button
                    ...
                    className="w-1/3 py-2 bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-semibold rounded-xl text-xs"
                  >
                    Cancel
                  </button>
                )}
                <button
                  type="submit"
                  className="flex-1 py-2 bg-brand-600 hover:bg-brand-500 text-white font-semibold rounded-xl shadow text-xs"
                >
      ...
      {/* Amount Slabs Tab — WITH EDIT BUTTON & FORM */}
      {activeTab === 'interest' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
            <table className="w-full text-left border-collapse text-sm">
                  ...
                  <tr key={s.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end space-x-1">
                        ...
                        {/* REQUIREMENT 4 FIX: EDIT ICON ADDED TO AMOUNT INTEREST SLABS */}
                        <button
                          onClick={() => {
                            setEditingInt(s);
                            setIntForm({
                          ...
                          className="p-1.5 rounded-lg text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-950/60"
                          title="Edit Amount Interest Slab"
                        >
                          <Edit className="w-4 h-4" />
                        </button>
          ...
          {/* Add/Edit Amount Interest Form — EXACT 4 REQUIRED FIELDS */}
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-4">
            <h3 className="font-bold text-slate-900 dark:text-slate-100 text-sm">
              {editingInt ? 'Edit Amount Interest Slab' : 'Add New Amount Interest Slab'}
            </h3>
            <form onSubmit={handleAddIntSlab} className="space-y-3 text-xs">
              <div>
                <label className="block font-semibold text-slate-600 mb-1">Slab Name * (Req #1)</label>
                <input
                  type="text"
                  required
                  placeholder="e.g. Micro Loans (0 - 1 Lakh)"
                  value={intForm.name}
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block font-semibold text-slate-600 mb-1">From Amount (₹) * (Req #2)</label>
                  <input
                    type="number"
                    required
                    placeholder="From ₹"
                    value={intForm.fromAmount}
                    onChange={(e) => setIntForm({ ...intForm, fromAmount: e.target.value })}
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2"
                  />
                </div>
                <div>
                  <label className="block font-semibold text-slate-600 mb-1">To Amount (₹) * (Req #3)</label>
                  <input
                    type="number"
                    required
                    placeholder="To ₹"
                    value={intForm.toAmount}
                    onChange={(e) => setIntForm({ ...intForm, toAmount: e.target.value })}
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  required
                  placeholder="Interest Rate (% p.a.)"
                  value={intForm.interestRate}
                  onChange={(e) => setIntForm({ ...intForm, interestRate: e.target.value })}
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2 font-bold text-emerald-600"
                />
              </div>
              ...
              <div className="flex space-x-2 pt-2">
                ...
                <button
                  ...
                  className="flex-1 py-2 bg-brand-600 hover:bg-brand-500 text-white font-semibold rounded-xl shadow text-xs"
                >
                  {editingInt ? 'Update Amount Slab' : 'Save Amount Slab'}
                </button>
              </div>
            </form>
          </div>
